...
date="201013"         # Need to be the same as the running date
...
### Edit Lasse
build_cases="T"                # Switch to only build cases without running for testing purposes
...
######## Build cases only, Lasse edit. Assumes that input data of SeedClim sites is already in place.
if [ ${build_cases} == "T" ]
then
  module purge
  cd ~/ctsm/cime/scripts
  ./create_newcase --case ../../../ctsm_cases/${plotname[i]}_4PFTs_1.1.1 --compset 2000_DATM%1PTGSWP3_CLM50%FATES_SICE_SOCN_MOSART_SGLC_SWAV --res 1x1_${plotname[i]} --machine saga --run-unsupported --project nn2806k

  cd  ~/ctsm_cases/${plotname[i]}_4PFTs_1.1.1 
  ./case.setup
  ./case.build
  ./case.submit
